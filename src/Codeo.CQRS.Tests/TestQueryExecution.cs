...
public class TestQueryExecution : TestFixtureRequiringData
{
    ...
    public class WhenQueryingWithJoinsRequiringPerRowMapping : TestFixtureRequiringData
    {
        ...
        private static int CreateTagForDepartment(
            int departmentId,
            string tag
        )
        {
    ...
    public class WhenTransactionIsRequired : TestFixtureRequiringData
    {
        ...
        public void ShouldNotThrowIfAvailable()
        {
            ...
            // Act
            using (var scope =
                   TransactionScopes.ReadCommitted(
    ...
    {
        [TestFixture]
        public class ForCommands
        {
            [Test]
            public void ShouldValidateTransactionOnRequest()
            {
                // Arrange
                Expect(Transaction.Current)
                    .To.Be.Null("should start without a transaction");
                var cmd = new CommandManuallyRequiringTransaction();
                // Act
                Expect(() => CommandExecutor.Execute(cmd))
                    .To.Throw<TransactionScopeRequired>();
                using var _ = TransactionScopes.ReadCommitted();
                Expect(() => CommandExecutor.Execute(cmd))
                    .Not.To.Throw();
                // Assert
            }
            ...
            [Test]
            public void ShouldValidateTransactionForDecoratedExecutor()
            {
                // Arrange
                Expect(Transaction.Current)
                    .To.Be.Null("should start without a transaction");
                var cmd = new CommandDecoratedToRequireTransaction();
                // Act
                Expect(() => CommandExecutor.Execute(cmd))
                    .To.Throw<TransactionScopeRequired>();
                using var _ = TransactionScopes.ReadCommitted();
        ...
        public class ForQueries
        {
            ...
            public void ShouldValidateTransactionOnRequest()
            {
                ...
                Expect(Transaction.Current)
                    .To.Be.Null("should start without a transaction");
                var cmd = new QueryManuallyRequiringTransaction();
                // Act
                Expect(() => QueryExecutor.Execute(cmd))
                    .To.Throw<TransactionScopeRequired>();
                using var _ = TransactionScopes.ReadCommitted();
                Expect(() => QueryExecutor.Execute(cmd))
                    .Not.To.Throw();
            ...
            public void ShouldValidateTransactionForDecoratedExecutor()
            {
                ...
                Expect(Transaction.Current)
                    .To.Be.Null("should start without a transaction");
                var cmd = new QueryDecoratedToRequireTransaction();
                // Act
                Expect(() => QueryExecutor.Execute(cmd))
                    .To.Throw<TransactionScopeRequired>();
                using var _ = TransactionScopes.ReadCommitted();
        ...
        [RequiresTransaction]
        public class CommandDecoratedToRequireTransaction : Command<int>
        {
        ...
        public class CommandManuallyRequiringTransaction : Command<int>
        {
            private static int _result;
        ...
        [RequiresTransaction]
        public class QueryDecoratedToRequireTransaction : Query<int>
        {
            private static int _result;
        ...
        public class QueryManuallyRequiringTransaction : Query<int>
        {
            private static int _result;
    ...
    {
        [TestFixture]
        public class WhenNotDecorated : TestQueryExecution
        {
            [Test]
        ...
        public class CachingQueries
        {
            ...
            public void ShouldAutomaticallyExpire()
            {
                ...
                Expect(memCache.Count)
                    .To.Equal(
                        0,
                        () => $"Cached keys: {memCache.Keys.JoinWith(",")}"
                    );
    ...
    public class WhenSpecifiedCacheKeyPropsArePrivate : TestFixtureRequiringData
    {
        ...
        public void ShouldCacheByThatKey()
        {
            ...
            Expect(FindPersonById(id).Name)
                .To.Equal(updated, "record should have been updated in the database");
            Expect(result1.Name)
                .To.Equal(name, "first query should have original name");
            Expect(result2.Name)
                .To.Equal(name, "should have retrieved from the cache");
            Expect(result3.Name)
                .To.Equal(another, "should not have retrieved from the cache");
        }
    }
